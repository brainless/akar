...
**Status:** Research Complete — Implementation Not Started (Tasks 1-6 researched below; new implementation Tasks 7-14 added for a future agent, none implemented)
...
**Findings:**
- `info`, `success`, `warning`, and `error` are the semantic "status" colors and their paired `*_content` tokens (used as text/icon color on top of a status-colored fill, e.g. a solid alert banner or a solid status badge) **fail normal-text AA in both themes, identically** — the status colors themselves are unchanged between `AKAR_THEME_DARK` and `AKAR_THEME_LIGHT` (`info: 0x3b82f6ff`, `success: 0x22c55eff`, `warning: 0xf59e0bff`, `error: 0xef4444ff` are byte-identical in both presets, confirmed by direct comparison of `theme.rs:55-62` and `theme.rs:93-100`), so this is a single fix applicable to both themes at once, not two separate fixes. `success_content`/`warning_content` fail even the *large-text/UI* 3:1 minimum (2.28:1 and 2.15:1) — these are the most severe failures found. Any component that renders white text/icons directly on a `success`- or `warning`-filled background (e.g. a solid success/warning alert or badge, per `alert.rs`/`badge.rs`) is very likely below AA today, regardless of font size.
...
- Action dispatch / `ActionRequest` handling (screen-reader-initiated clicks, increment/decrement, set-value) flowing back into component state — akar's component contract (`AGENTS.md` → Component contract) has components read caller-owned state and mutate it synchronously within the same call; AccessKit's `ActionHandler` is a callback fired asynchronously from the platform adapter's thread/runtime loop, which does not fit that model today and needs its own design pass, not assumed solved by this epic.
...
- Darken `success`/`warning` (or lighten their `*_content` counterparts away from pure white) until `success_content`-on-`success` and `warning_content`-on-`warning` clear at least 3:1 (ideally 4.5:1, matching `info`/`error`'s current ~3.7:1 as a floor, but 4.5:1 is the real AA target for arbitrary-size text on these fills). Re-run the same luminance/contrast computation used in Task 5 (formula documented there) against the new values before committing — do not eyeball hex values.
- Separately: `primary` on `base_100` in the dark theme (1.11:1) and `secondary` on `base_100` in the light theme (1.10:1) fail even the 3:1 UI-component-boundary minimum. Decide whether this needs a fix (it matters only if/when a focus ring or outline-variant border is drawn in `primary`/`secondary` directly against `base_100` — check `button.rs`'s `ButtonVariant::Outline` path and any future focus-ring implementation from Task 8) or a new dedicated token (e.g. `focus_ring`) that doesn't reuse `primary`/`secondary`, sidestepping the conflict with those tokens' other roles.
...
- Add unit tests in `a11y.rs` following the existing `draw_list.rs` test style (`crates/akar-core/src/draw_list.rs:196-329`): frame-clearing behavior, inactive-frame no-op behavior, and a basic push/read round trip. Use `AkarCore::mock()` (`context.rs:48-60`) as the existing tests for `button.rs` do.
...
- Populate `A11yNode::parent_id` using `Layout`'s existing `parents: HashMap<NodeId, NodeId>` (`crates/akar-layout/src/lib.rs:35`) via `layout.widget_id(parents[&node_id])`, rather than leaving it `None` as the Task 3 sketch did — real parent/child linkage is needed for AccessKit's `Node::children` field on the eventual `accesskit::Node`, which is built in Task 11 from this data.
- Add `MockDrawList`-style tests (per `AGENTS.md` → Testing approach) asserting the right `A11yNode` is pushed with the right role/label/state for each of the six components, using `AkarCore::mock()` the same way `button.rs`'s existing tests do (`button.rs:174-257`).

### Task 11 — `accesskit_winit` Bridge in `akar-winit`

**Status:** Not Started

- `akar-winit` currently has zero window-handle-holding state (`crates/akar-winit/src/lib.rs` is 212 lines, entirely the stateless `process_window_event(input: &mut InputState, event: &WindowEvent)` function plus small pure key-mapping helpers). This task introduces a new stateful bridge type (e.g. `AkarWinitA11yBridge` or folded into a new `AkarWinitState` if one doesn't already exist elsewhere in the demo/example code) that owns an `accesskit_winit::Adapter`, gated behind a new Cargo feature (e.g. `accesskit`, matching `egui-winit`'s `accesskit = ["dep:accesskit_winit"]` pattern at `egui-winit/Cargo.toml:29-30`) so `akar-winit` has no new dependency by default.
- Respect `accesskit_winit::Adapter::with_event_loop_proxy`'s documented sequencing constraint (`accesskit_winit-0.32.2/src/lib.rs:131-152`): the adapter must be constructed before the window is first shown (create window with `with_visible(false)`, construct the adapter, then show the window) — this is an ordering requirement on whatever example/demo code exercises this bridge (likely `examples/demo-rust`), not just the bridge itself.
- Each frame, after `AkarCore::end_frame` (Task 8's `A11yTree` is now populated), translate `A11yTree::nodes()` into a real `accesskit::TreeUpdate` (per Task 1's findings: `nodes: Vec<(NodeId, Node)>`, `tree: Option<Tree>`, `tree_id: TreeId`, `focus: NodeId` — the `focus` field should be sourced from Task 9's focus-sequence state) and call the adapter's update path (mirroring `egui-winit`'s `accesskit.update_if_active(|| update)`, `egui-winit/src/lib.rs:1168-1174`, so the translation closure only runs when accessibility is actually active).
