...
### 014c — Structured frame dump (--dump-frame) (merged)
- **Files:** `crates/akar-core/src/draw_list.rs` (recording mode + serde-gated derives), `crates/akar-core/Cargo.toml` (optional `serde` feature), `examples/demo-rust/Cargo.toml` (`serde`/`serde_json` + enables `akar-core/serde`), `examples/demo-rust/src/main.rs` (`--dump-frame` + `FrameDump`/`InputSnapshot`), `Cargo.lock`.
- **Verdict:** Implemented as specified. `DrawList` gained `recording: bool` + `recorded: Vec<RecordedCall { call, scissor }>`. Recording snapshots the call BEFORE the scissor-cull early-return, so culled calls are captured (verified by `recording_captures_culled_quad`). `QuadCall`/`TextCall` and the 112-byte Pod assert are untouched. `serde::Serialize` is gated behind a new optional `serde` feature on `akar-core` (non-Rust consumers pay nothing). `--dump-frame <PATH>` serializes recorded calls, labeled rects, and an input snapshot to JSON; works standalone or alongside `--screenshot`. Tests pass; `clippy -D warnings` (workspace, lib/bins) and `fmt` clean.
- **Note:** `cargo clippy --workspace --tests -- -D warnings` surfaces a pre-existing `len_zero` lint in `akar-components` test code unrelated to this epic; left untouched. The `InputSnapshot` mirrors `InputState` on the demo side (glam `Vec2`→`[f32;2]`, `Key`→`Debug` string) to avoid a `glam` serde feature requirement on `akar-core`.

### 014d — Multi-capture and comparison (akar-diff) (merged)
