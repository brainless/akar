...
- [ ] `cargo test --workspace` passes (unit tests from Tasks 2, 3, 7).
- [ ] `cargo clippy --workspace -- -D warnings` passes (or documented suppressions).
- [ ] `cargo run --manifest-path examples/demo-rust/Cargo.toml` renders a visible, hoverable, clickable button.
- [ ] `akar.h` is generated by cbindgen and compiles as C.
- [ ] No windowing or event loop code exists in `akar-core` or `akar-components`.
- [ ] No async code exists anywhere in the workspace.
