...
**Status:** Research Complete and Verified — Implementation Not Started. Tasks 1-4 (research) are Done and were re-verified on 2026-08-12 against current source, this time with execution-based evidence on a machine with a real GPU and display; the original file's "no live GPU/display in this research environment" premise no longer applies and has been corrected throughout. Implementation Tasks 5, 6, 7 and 9 are `Ready for implementation`; Task 8 is `Blocked` on a working scripted text-entry path in `demo-rust` (see "Review — 2026-08-12 verification pass"). No implementation code has been written.
...
- **Two further shaping facts a fix must not trip over**, from the same probe:
  ...
  - Several `LayoutGlyph`s can share an identical `start..end`. Devanagari `"\u{0915}\u{094D}\u{0937}\u{093F}"` yields two glyphs both spanning `0..12`; Thai `"\u{0E01}\u{0E33}"` yields two glyphs both spanning `3..6` with widths `0.00` then `9.51`; `"e\u{0301}\u{0301}\u{0301}"` yields three glyphs all spanning `0..7` with widths `8.20, 0.00, 0.00`. `caret_x` (`text_pipeline.rs:366-369`) selects with `.find(|glyph| index >= glyph.start && index <= glyph.end)`, i.e. the **first** match, which for the Thai case is the zero-width mark glyph. Whether that first-match rule is correct is a pre-existing question that this task should resolve or explicitly document rather than silently inherit.
...
**Readiness:** Blocked — needs a working scripted text-entry path in `demo-rust`, plus CJK sample text in the demo (or an equivalent `--component` fixture). Details and a known-good workaround in the review section under "Task 8 blockers". This is a demo/tooling gap, not a research gap; the underlying behaviour claims of Tasks 1 and 2 have both been verified by execution already, so Task 8 is now about producing durable before/after artifacts rather than about establishing the facts.

- Tasks 1 and 2 have since been verified by execution (see the review section); what this task still owes is captured before/after evidence in the repo's own toolchain. Blockers found on 2026-08-12:
  ...
  - The demo contains no CJK text and no CJK-capable fixture, so the `paragraph`/`textarea` CJK screenshot cannot be captured without adding sample content to `examples/demo-rust` — a source change, hence implementation-phase work.
