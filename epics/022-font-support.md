...
Tasks 5 and 7 below are updated to reflect these as adopted decisions rather than open blockers.
...
**Status:** Not Started
**Readiness:** Dependency-blocked on Task 5's API amendment; otherwise ready. The font choice, license, and asset location are resolved. Land this together with Task 5.
...
- **Decided 2026-08-12 (was an open question):** bundle **IBM Plex Sans Regular**, SIL OFL 1.1. Source file and license text are both available locally at `~/Projects/zed/assets/fonts/ibm-plex-sans/{IBMPlexSans-Regular.ttf,license.txt}`; the license text was read in full during the verification pass. OFL 1.1 permits embedding and redistribution with any software provided the notice and license travel with it (`license.txt:48-51`, `53-61`), forbids selling the font by itself, forbids reusing the reserved name "Plex" for modified versions, and requires the font stay under OFL — none of which conflicts with akar's MIT `LICENSE`, which governs akar's code, not the font binary. Alternative verified locally: Lilex (OFL 1.1, `~/Projects/zed/assets/fonts/lilex/OFL.txt`) if a monospace default is also wanted. Avoid egui's `Ubuntu-Light.ttf` (Ubuntu Font Licence) and `Hack-Regular.ttf` (bespoke licence) — more restrictive and less standard. Caveat: these license texts are the copies distributed in those local checkouts and were not cross-checked upstream.
- **Asset location decided:** no font asset convention exists in this repo (no `*.ttf`/`*.otf`/`*.ttc`/`*.woff*` file exists anywhere outside `target/`). Create `crates/akar-core/assets/fonts/` — `include_bytes!` must resolve relative to the embedding crate — and put the OFL text next to the font. Add a `NOTICE` or a bundled-font paragraph in `LICENSE` recording the OFL obligation. Load via `include_bytes!` into the default `TextPipelineConfig::fonts` used by `AkarCore::new`'s default path and by `AkarCore::mock()`.
